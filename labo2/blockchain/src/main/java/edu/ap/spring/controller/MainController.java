...
public class MainController {
	...
	private final Wallet walletA;
	private final Wallet walletB;

    public MainController(BlockChain bChain, Wallet coinbase, Wallet walletA, Wallet walletB) {
        this.bChain = bChain;
    ...
    public String getBalance(@PathVariable("wallet") String wallet,
        ...
        if (w != null) {
            model.addAttribute("balance", w.getBalance());
        } else {
    ...
    public String transaction(@RequestParam("wallet1") String wallet1,
        ...
        try {
            if (wallet1.equalsIgnoreCase("walletA") && wallet2.equalsIgnoreCase("walletB")) {
                ...
                walletA.sendFunds(walletA.getPublicKey(), amount);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
